...
const ColorPicker: FunctionComponent = () => {
    ...
    const [isPickerAnimated, setIsPickerAnimated] = useState<boolean>(false);
    const [color, setColor] = useState<string>(localStorage.getItem("primary-color") || "#ff4d00");
    ...
    const handleThemeClicked = () => {
        if(!isPickerAnimated){
            setIsPickerAnimated(true);
            setShowPicker(!showPicker);
            setTimeout(() => {
                setIsPickerAnimated(false);
            },1000)
        }
    }
    ...
    return(
        <div className="color-picker">
            <Button className={`btn-invisible theme-btn ${showPicker ? "picker-open" : "picker-close"}`}>
                <p onClick={handleThemeClicked}>Theme</p>
                {(isPickerAnimated || showPicker) && <BlockPicker triangle="hide" color={color} onChange={(e: any) => {handleColorChange(e.hex)}} colors={defaultColors}/>}
            </Button>
